 ...
 * @param warn_duration Duration after which to warn if the frames are not
 * available
 * @param warn_interval Interval in which to keep warning if the frames are not
 * available
...
void wait_for_tf(const rclcpp::Logger &logger, std::shared_ptr<rclcpp::Clock> clock,
  ...
  while (rclcpp::ok()) {
    try {
      ...
      // Apply tf_buffer->_frameExists to all frames and check if all are
      // available otherwise return false (functional) We use _frameExists
      // because it does not spam the console with errors if the frame does not
      // exist...
      ...
      if (!std::all_of(frames.begin(), frames.end(), [tf_buffer, root_frame, check_interval](const std::string frame) {
        ...
        // Here it is fine not to wait as the canTransform function already
        // includes a timeout Retry
        continue;
      }
